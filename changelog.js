...
let justNubmers = string.match(regex1);
console.log(justNubmers);


//TASK 2 
const text = "City postal codes: 12345, 98765, 54321";
const regex2= /\b\d{5}\b/g;

const postalIndex= text.match(regex2);
if (postalIndex) {
  console.log("Действительные почтовые индексы:", postalIndex);
} else {
  console.log("Нет действительных почтовых индексов в строке.");
}
...
movies.forEach(function(movie) {
  let movieYear = String(movie.year); 
  let match = movieYear.match(regex3);
  if (match) {
    modernMovies.push(movie);
  }
});

console.log(modernMovies);
...
Object.keys(colorsObj).forEach(function(colorName) {
  const colorCode = colorsObj[colorName];
  const colorCodes = colorCode.match(colorCodeRegex);
  if (colorCodes) {
    console.log(`Color: ${colorName}, Code: ${colorCodes[0]}`);
  }
});
